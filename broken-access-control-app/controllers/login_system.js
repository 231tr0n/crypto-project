...
controllers.login = (request, response) => {
	if (request.body.username && request.body.password) {
		db.query('SELECT * FROM users WHERE username = ? AND password = ?', [request.body.username, request.body.password], (error, results, fields) => {
			if (error) {
				console.log(error);
				...
				if (results.length == 1) {
					...
					response.cookie('admin', results[0].admin, {
						httpOnly: true,
						sameSite: true
					});
